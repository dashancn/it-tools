...
<template>
  <div>
    <c-card>
      <c-input-text v-model:value="input" label="输入数字" placeholder="在这里输入数字（例如：42）" label-position="left" label-width="110px" mb-2 label-align="right" />

      <n-form-item label="输入进制" label-placement="left" label-width="110" :show-feedback="false">
        <n-input-number v-model:value="inputBase" max="64" min="2" placeholder="在这里输入原始进制（例如：10）" w-full />
      </n-form-item>
      ...
      <div flex items-baseline>
        ...
        <InputCopyable
          ...
          :placeholder="`Base ${outputBase} 结果会显示在这里...`"
        />
      </div>
    </c-card>
  </div>
